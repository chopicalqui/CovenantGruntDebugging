...
/**
    GruntExecutor.Grunt.Execute(GUID
        , SessionKey
        , ProfileReadFormat
        , ProfileWriteFormat
        , pipe
...
namespace GruntExecutor
{
    class Grunt
    {
        public static void Execute(string GUID
            , Aes SessionKey
            , string ProfileReadFormat
            , string ProfileWriteFormat
            , NamedPipeServerStream ServerPipe = null
            , string PipeName = null)
        {
            try
            {
                int Delay = 5; //Convert.ToInt32(@"{{REPLACE_DELAY}}");
